...
Two, in [`hooks/hooks.json`](./hooks/hooks.json). They are live as long as the plugin is
enabled, and there is nothing to paste into your settings.

| Event | What fires | What it costs you |
|---|---|---|
| `UserPromptSubmit` | `nex_search` on your prompt, five hits | one read against your own tenant |
| `SessionEnd` | `nex_session_end`, closing the memory session | one write, no arguments |

Read this before you enable it: the first hook sends **every prompt you type** to
`memory.studiomeyer.io`, into your own tenant. That is what "recall on every prompt"
means, and it is the reason the plugin is worth having. If you do not want it, disable
the plugin or delete that entry from `hooks/hooks.json` in your installed copy.

Two things this plugin deliberately does **not** hook: `nex_summarize` needs a written
summary that only the model can produce, and a hook cannot write prose. And closing the
session on `Stop` would close it after every single answer, not at the end of your work.
