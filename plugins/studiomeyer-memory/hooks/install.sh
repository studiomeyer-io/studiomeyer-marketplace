#!/usr/bin/env bash
# StudioMeyer memory — Hook Recipe Installer
...
PLUGIN_NAME="studiomeyer-memory"
...
case "$MODE" in
  install)
    ...
    echo "(Type a prompt, Ctrl-D, watch for 'Memory: summarizing session...' in the statusline.)"
    ;;
